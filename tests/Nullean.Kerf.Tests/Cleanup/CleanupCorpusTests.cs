...
/// <summary>
/// The adversarial gate: feed a real corpus verdicts Kerf has no business trusting, and require that it
...
/// </para>
/// <para>
/// The point is that a wrong verdict must still not produce broken output. Kerf's premise is that it trusts
...
/// </para>
/// <para>
/// Skipped unless <c>KERF_CLEANUP_CORPUS</c> names a directory, the same shape as the expectation dump.
...
public class CleanupCorpusTests
{
	private static string? Corpus =>
		Environment.GetEnvironmentVariable("KERF_CLEANUP_CORPUS") is { Length: > 0 } path && Directory.Exists(path)
			? path
			: null;
	...
	public async Task No_wrong_verdict_produces_damage()
	{
		...
		{
			Console.WriteLine("no KERF_CLEANUP_CORPUS; run ./build.sh cleanupsafety --corpus <path>");
			return;
		}
		...
		if (Environment.GetEnvironmentVariable("KERF_CLEANUP_SUMMARY") is { Length: > 0 } destination)
