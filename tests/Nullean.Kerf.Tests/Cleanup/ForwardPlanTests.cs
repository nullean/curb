...
public class ForwardPlanTests
{
	...
	[Test]
	public async Task A_rule_kerf_never_fixes_is_not_forwarded_either()
	{
		// Kerf's position on these is not "we cannot" but "no tool should do this unattended", so routing
		// around it through another tool would be Kerf ignoring its own judgement.
	...
	[Test]
	public async Task A_refusal_that_is_only_kerfs_is_still_forwarded()
	{
		// IDE0051 deletes a declaration, which Kerf will not do. `dotnet format style` will, and someone
		// who escalated the rule and asked to forward has said they want it — so Kerf's own constraint is
