...
/// <remarks>
/// <para>
/// A real .NET key that <c>dotnet format style</c> implements only with a workspace behind it. Kerf
/// reaches the same answer from syntax alone, and the expectations here were checked against the real
/// thing: on the combined case, Kerf's output is byte-identical to <c>dotnet format style</c> followed
...
/// inside the block stops being scoped to it — while adding braces cannot. Kerf only adds.
/// </para>
/// <para>
...
public class PreferBracesTests : FormattingTest
{
	...
	public Task False_does_not_take_braces_off() => Unchanged(
		// Roslyn would remove them. Kerf does not: a declaration inside the block stops being scoped
	...
	public Task The_brace_style_option_still_governs_where_the_brace_goes() => Formats(
		...
		""",
		// The brace Kerf synthesises is placed by the same option as every brace the source wrote, so
