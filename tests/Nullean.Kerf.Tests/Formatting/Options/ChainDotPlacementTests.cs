...
public class ChainDotPlacementTests : FormattingTest
{
	...
	public Task A_break_is_kept_whichever_side_the_dot_was_on() => Formats(
		// Both chains are broken; they differ only in where the author left the dot. Kerf used to keep
		...
		// head of the line, which is Kerf's rendering; the break itself is the author's and stays.
		"""
		public class C
		{
		    void Leading()
	...
	public Task A_receiver_that_keeps_its_first_call_keeps_that_dot_glued() => Formats(
		// Where Kerf introduces the breaks itself, a plain identifier receiver keeps its first call —
