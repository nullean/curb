...
public class FormattingRunCacheTests
{
	...
	/// <summary>Formats until nothing moves, so a test can start from files Kerf agrees with.</summary>
	...
	{
		// The rule the whole design turns on. Kerf is idempotent, so recording what it just wrote would
	...
	[Test]
	public async Task Files_kerf_was_told_to_leave_alone_stay_skipped_rather_than_becoming_cached()
	{
	...
	{
		// The case the feature earns the most in. Kerf_Check=true does not stamp on failure, so the
