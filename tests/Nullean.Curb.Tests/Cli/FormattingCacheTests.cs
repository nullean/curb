...
public class FormattingCacheTests
{
	...
	[Test]
	public async Task A_cache_written_by_another_version_of_curb_is_discarded()
	{
		...
		lines[0] = "curb-cache 1 0.0.1-someone-elses-build";
	...
	public async Task A_cache_written_in_another_line_format_is_discarded()
	{
		var fs = Saved(Now);
		var lines = fs.File.ReadAllLines(CachePath);
		lines[0] = lines[0].Replace("curb-cache 1 ", "curb-cache 2 ", StringComparison.Ordinal);
		fs.File.WriteAllLines(CachePath, lines);
	...
	public async Task Rubbish_in_the_file_is_not_an_error()
	{
		...
		// the work.
		foreach (var contents in new[] { "", "\0\0\0\0", "not a header at all", "curb-cache" })
		{
			var fs = Repo();
	...
	public async Task Saving_creates_the_directory_and_leaves_no_temporary_behind()
	{
		...
		var cache = FormattingCache.Load(fs, "/repo/nested/deeper/curb.cache", Now);
		cache.Record("/repo/A.cs", Content, Options);
		cache.Save();

		fs.File.Exists("/repo/nested/deeper/curb.cache").Should().BeTrue();
		fs.File.Exists("/repo/nested/deeper/curb.cache.tmp").Should().BeFalse();
		await Task.CompletedTask;
	}
