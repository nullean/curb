...
/// <remarks>
/// <para>
/// All three are documented with a default of true and Curb defaults all three to false. That is
...
/// False keeps Curb a fixed point of dotnet format and leaves the expanded layout one line away.
/// </para>
/// <para>
...
/// member per line <em>once the initializer is opened</em>, and Curb read it as a licence to open
...
public class MemberSeparatorTests : FormattingTest
{
	...
	public Task The_option_is_about_expanding_not_about_joining() => Unchanged(
		// Turning it off does not gather up a layout the author chose; nothing in Curb joins lines,
	...
	public Task The_object_initializer_key_reaches_anonymous_types_too() => Formats(
		...
		// still spreads an opened-out anonymous type. Curb had the two keys independent.
		"""
		public class C
		{
		    public void M()
	...
	public Task A_long_query_still_breaks_with_the_option_off() => Formats(
		...
		// what Curb did rather than what dotnet format does — the difference only shows when the
