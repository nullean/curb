...
/// <remarks>
/// <para>
/// The fourth real .NET key Curb applies without a compilation, and the one with the largest visible
...
/// <c>#pragma</c> pair straddling the declaration — Curb's output is byte-identical to
...
public class NamespaceStyleTests : FormattingTest
{
	...
	public Task Block_scoped_is_accepted_and_changes_nothing() => Unchanged(
		// Curb never puts the braces back, so this value is inert rather than unsupported. The
	...
	public Task A_declaration_with_nothing_under_it_gains_the_blank_line() => Formats(
		// dotnet format inserts this, so Curb does too — with or without the key, since it is
		// agreement rather than an opinion. Missing it was a real gap: the first project to consume
		// the MSBuild package still failed IDE0055 on exactly this line after Curb had run.
		"""
		namespace Alpha;
		public class C
		{
