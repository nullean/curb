...
/// fixed point, and Format Document would undo it on every save — the one thing Curb's defaults exist
...
public class AttributeTests : FormattingTest
{
	...
	public Task Sections_on_a_member_each_take_a_line() => Formats(
		// dotnet format does this too, so the split is not an opinion Curb holds alone.
		"""
		public class C
		{
	...
	public Task A_section_sharing_a_line_with_its_member_is_split_off() => Formats(
		...
		""",
		// dotnet format leaves this one alone, but it never rejoins what Curb splits, so splitting is
		// a safe opinion and one Curb holds by default.
		"""
		public class C
		{
		    [Obsolete]
	...
	public Task Sections_on_a_parameter_stay_glued() => Unchanged(
		// `[A] [B]` is what ReSharper would write. dotnet format actively removes that space, so Curb
	...
	public Task A_space_between_sections_on_a_parameter_is_removed() => Formats(
		// The direction that matters: given the spaced form, Curb normalises to glued exactly as
	...
	/// here without the mode it needs reports CURB1005 and changes nothing.
	/// </remarks>
	[Test]
