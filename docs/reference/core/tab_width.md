...
- **ide0055**: Same not-fully-root-caused divergence as the dotnet-format-whitespace entry above (indent_style = tab combined with a width narrow enough to force wrapping), now confirmed to reproduce under a real `dotnet build -p:EnforceCodeStyleInBuild=true` as well as the `dotnet format whitespace` CLI. (`CoreOptionTests.Tab_width_counts_toward_the_line_length`)
