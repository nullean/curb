	...
	string SnippetKey)
{
	...
	[
		...
		new("csharp_wrap_chained_binary_expressions", OptionGroup.Wrapping, ["chop_if_long", "wrap_if_long"], "(not set)", "Break each operand of a long && or || chain onto its own line. wrap_if_long packs operands instead of breaking every one. Only honoured in deterministic mode (max_line_length set).", "binary_ops"),
		...
		new("csharp_wrap_object_and_collection_initializer_style", OptionGroup.Wrapping, ["chop_always", "chop_if_long", "wrap_if_long"], "chop_if_long", "chop_always gives every initializer element its own line; wrap_if_long packs elements instead. Only honoured in deterministic mode.", "object_init"),
