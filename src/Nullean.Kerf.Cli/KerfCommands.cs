...
/// <summary>
/// The command surface. One public method per <c>kerf</c> command; <see cref="Nullean.Argh.ArghApp.Map{T}"/>
...
/// </remarks>
internal sealed class KerfCommands
{
	...
	/// </summary>
	/// <remarks>
	/// Cleanup never builds. Build first, then run it: <c>dotnet build &amp;&amp; kerf cleanup</c>.
	/// </remarks>
	/// <param name="path">Where to look for <c>kerf.sarif</c> logs, and which files to clean.</param>
	/// <param name="sarifLogs">
	/// -s, --sarif-log, The log to read, instead of searching <paramref name="path"/> for
	/// <c>kerf.sarif</c>. May be given more than once, so a solution's per-project logs can all be
	...
	/// <param name="forward">
	/// Hand what Kerf cannot fix to <c>dotnet format</c>, scoped to the reported rules and files, and
	/// report both timings.
	/// </param>
	public int Cleanup(
		[Argument] string path = ".",
	...
	/// <summary>Show which code style rules Kerf fixes, and which it does not.</summary>
	/// <param name="cleanupIds">Print only the ids <c>kerf cleanup</c> fixes, space separated, for scripting.</param>
	...
	public int DocTree([Argument][Existing][FileExtensions(Extensions = "cs")] FileInfo path)
	{
		...
		var options = EditorConfigOptionsBinder.Bind(new KerfEditorConfig(_fileSystem).For(full));
	...
	{
		var full = _fileSystem.Path.GetFullPath(path.FullName);
		var config = new KerfEditorConfig(_fileSystem).For(full);
