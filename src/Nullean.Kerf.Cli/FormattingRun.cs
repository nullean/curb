...
internal static class FormattingRun
{
		...
		string[]? explicitFiles = null)
	{
		...
		// Decouple I/O from formatting. All workers calling ReadAllBytes simultaneously causes
		// kernel-level serialisation on small-file reads (kernel VFS lock contention, not disk
		// saturation). 4 concurrent readers is enough to fill the page cache on local NVMe without
		// triggering that pressure; the right bound is I/O path depth, not CPU core count.
		using var ioGate = new SemaphoreSlim(4);
		...
		Parallel.ForEach(
			...
			(item, _, formatter) =>
			{
				...
				try
				{
					bytes = fileSystem.File.ReadAllBytes(item.Path);
				}
				finally
				{
					ioGate.Release();
