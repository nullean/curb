...
/// <remarks>
/// <para>
/// An entry means one thing: Curb ran the formatter over exactly these bytes under exactly these resolved
/// options and got them back unchanged. An observation, never an inference — which is why a file Curb
...
/// so far grew a blank line or an indent level per run. Today a second <c>curb format</c> changes such a
...
internal sealed class FormattingCache
{
	...
	private const string Marker = "curb-cache";
	...
	/// printer while passing <c>--cache</c> can read stale results. The cache is opt-in, Curb's own build
